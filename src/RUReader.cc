...
RUReader::RUReader( std::map<int,std::string> name, std::string inFile, std::string outFile ){
  ...
  for( auto it : dgtzChans ){ 
    ...
    for(UInt_t j = 0 ; j < channels; j++){
      if( dgtzDppType[bIdx] == CAEN_DGTZ_DPPFirmware_PHA ){
        fEnergy[bIdx].push_back(new TH1F(Form("ru_hEnergy_%i_%i",bIdx,j),
        ...
        fQshort[bIdx].push_back(new TH1F(Form("ru_hQshort_%i_%i",bIdx,j),
			          Form("Qshort Board %i Channel %i",bIdx,j),
			          32768,0,32768));
        fQlong[bIdx].push_back(new TH1F(Form("ru_hQlong_%i_%i",bIdx,j),
...
void RUReader::UnpackHeader( uint32_t* inpBuffer, uint32_t& aggLength, uint32_t& board, std::bitset<8>& channelMask, uint64_t& offset ){
...
void RUReader::UnpackPHA( uint32_t* inpBuffer, uint32_t& board, std::bitset<8>& channelMask, uint64_t& offset ){
  ...
  for(uint8_t cpl = 0 ; cpl < channelMask.count() ; cpl++){
    ...
    for(uint16_t evt = 0 ; evt < (coupleAggregateSize-2)/dataForm.EvtSize();evt++){
      ...
      format = dataForm.GetFormat( "TS" );
      tstamp = ( RO[board]<<(format.second+1) | tstamp );
...
void RUReader::UnpackPSD( uint32_t* inpBuffer, uint32_t& board, std::bitset<8>& channelMask, uint64_t& offset ){
  ...
  for(uint8_t cpl = 0 ; cpl < channelMask.count() ; cpl++){
    ...
    for(uint16_t evt = 0 ; evt < (coupleAggregateSize-2)/dataForm.EvtSize();evt++){
      ...
      if( dataForm.CheckEnabled( "Trace" ) && fUpdateWave ){
...
int RUReader::ReadHeader( std::ifstream& input ){
...
void RUReader::ReadData( std::ifstream& input, uint64_t pos ){
  ...
  while( pos < length && !stopCall ){
    ...
    emit updateProgress( (pos*perCent)/length );

    uint64_t offset = 0;
    while( offset < buffSize && !stopCall ){
      ...
      emit updateProgress( ((pos + offset*sizeof(uint32_t))*perCent)/length );
...
void RUReader::Write( ){
  ...
  // Creating ROOT file
  fileOut = new TFile( out.c_str( ), "RECREATE" ); 
